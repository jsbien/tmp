...
\pismoPL{Florian Ungler 3. Pismo tekstowe gotyckie. Krój M⁹¹. Stopień 20 ww. == 72/73 mm. — Tabl. 114."}
 ...
 \pismoEN{Florian Ungler 2. Gothic text font. Typeface M⁹¹. Type size 20 ww. == 72/73 mm. — Plate 114.}
...
\examplePage{\textit{Karta A₂b}}
  ...
  \bigskip
  \exampleLib{Biblioteka Jagiellohska. Kraków.}
...
2.1 Check the consistency of the directory content.

The directory should contain at least one file with `+` in the file
name, e.g. `m44_R_lines_01_chunk_13+14.png`. Abort the run with an error
 message if not empty.

 2.2. Copy the file(s) with `+` in the file name to the input
...
The directory should contain at least one file with. Abort the run
...
3.3. Ask the user if the output is acceptable and wai for an answer.
